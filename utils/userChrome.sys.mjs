console.log("userChrome.sys.mjs loaded");

const { xPref } = ChromeUtils.importESModule(
  'chrome://userchromejs/content/xPref.sys.mjs'
);
// const { Management } = ChromeUtils.importESModule('resource://gre/modules/Extension.jsm');
// const { AppConstants } = ChromeUtils.importESModule('resource://gre/modules/AppConstants.mjs');
const AppConstants = {
...
var _uc = {
  ...
  loadScript: function (script, win) {
    if (!script.regex.test(win.location.href)) {
      console.log(script.regex);
      console.log(win.location.href);
      console.log("Blocked by include:", script.filename, win.location.href);
      return;
    }

    if (script.filename != this.ALWAYSEXECUTE && !script.isEnabled) {
      console.log("Disabled:", script.filename);
      return;
    }
    ...
    try {
      let target = script.onlyonce ? { window: win, UC, _uc, xPref } : win;

      Services.scriptloader.loadSubScript(script.url + '?' + script.file.lastModifiedTime,
                                    script.onlyonce ? win : win);
      ...
      console.log("script error: " + script.filename + "\n" + ex);
      Cu.reportError(ex);
    }
  },
...
var UserChrome_js = {
  ...
  load: function (window) {
    ...
    if (/^(chrome:(?!\/\/global\/content\/commonDialog\.x?html)|about:(?!blank))/i.test(location.href)) {
...
globalThis.UC = UC;
globalThis._uc = _uc;
globalThis.UserChrome_js = UserChrome_js;
