...
(function () {
  ...
  function getOverlayContainer() {
    ...
    if (!host) {
      console.warn(
        "[Chat Buttons] Could not find a content host element to overlay.",
      );
      return null;
    }
  ...
  function getOverlayBrowser(def) {
    ...
    updateButtonLoadedState(def.id, true);
    return browserEl;
  }
  ...
  function restoreRealTabSelectionVisual() {
    ...
    if (visuallyselected !== null)
      tab.setAttribute("visuallyselected", visuallyselected);
  ...
  function showOverlay(def) {
    ...
    for (let key in overlays) {
      overlays[key].style.setProperty(
        "display",
        key === def.id ? "flex" : "none",
      );
    }
    container.style.setProperty("display", "block");
    selectButton(def.id);
    document.documentElement.setAttribute(
      "chat-launcher-overlay-active",
      "true",
    );
    hideRealTabSelectionVisual();
  }

  function updateButtonLoadedState(id, loaded) {
    let img = buttonImages[id];
    let btn = document.getElementById(id);

    if (!img || !btn) return;

    img.style.filter = loaded ? "brightness(1)" : "brightness(0.55)";
  }
    ...
    btn.addEventListener("contextmenu", (event) => {
      ...
      reloadItem.addEventListener("command", () => {
        try {
          ...
          console.warn("[Chat Buttons] Failed to reload browser", e);
        }
      });

      let destroyItem = document.createXULElement("menuitem");
      destroyItem.setAttribute("label", `Close`);

      destroyItem.addEventListener("command", () => {
        browserEl.remove();
        delete overlays[def.id];

        updateButtonLoadedState(def.id, false);
  ...
  function init() {
    ...
    if (!tabsToolbar) {
      console.warn(
        "[Chat Buttons] No titlebar-buttonbox and no #TabsToolbar found; skipping init.",
      );
      return;
    }
